...
namespace MealTicket_Web_Handler
{
    public class Shares_Tag_DayLeader_Session
    {
        ...
        public static Dictionary<long, Dictionary<long, Shares_Tag_DayLeader_Session_Info>> UpdateSession()
        {
            using (var db = new meal_ticketEntities())
            {
                DateTime dateNow = DateTime.Now.Date;
                string sql = string.Format(@"merge into t_plate_shares_rel_tag_dayleader_date as t
using (select * from t_plate_shares_rel_tag_dayleader) as t1
ON t.Market = t1.Market and t.SharesCode = t1.SharesCode and t.PlateId=t1.PlateId and t.[Type]=t1.[Type] and t.[Date]='{0}'
when matched
then update set t.DayLeaderType = t1.DayLeaderType
when not matched by target
then insert([Type],Market,SharesCode,PlateId,DayLeaderType,[Date]) values(t1.[Type],t1.Market,t1.SharesCode,t1.PlateId,t1.DayLeaderType,'{0}');", dateNow.ToString("yyyy-MM-dd"));
        ...
        private static Dictionary<long, Dictionary<long, Shares_Tag_DayLeader_Session_Info>> BuildDic(List<Shares_Tag_DayLeader_Session_Info> dataList)
        {
            Dictionary<long, Dictionary<long, Shares_Tag_DayLeader_Session_Info>> session_Dic = new Dictionary<long, Dictionary<long, Shares_Tag_DayLeader_Session_Info>>();
            foreach (var item in dataList)
            {
                long key1 = item.PlateId * 100 + item.Type;
                if (!session_Dic.ContainsKey(key1))
                {
                    session_Dic.Add(key1, new Dictionary<long, Shares_Tag_DayLeader_Session_Info>());
                }
        ...
        {
            var data = objData as Dictionary<long, Dictionary<long, Shares_Tag_DayLeader_Session_Info>>;
