...
{
    public class AccountTrendTriSession:Session<List<AccountTrendTriInfo>>
    {
        ...
        public override List<AccountTrendTriInfo> UpdateSession()
        {
            ...
            {
                lastList = new List<AccountTrendTriInfo>();
...
where t.num=1", dateNow.ToString("yyyy-MM-dd"), lastDataTime.Value.ToString("yyyy-MM-dd HH:mm:ss"));
                var list= db.Database.SqlQuery<AccountTrendTriInfo>(sql).ToList();
                if (list.Count() > 0)
                {
